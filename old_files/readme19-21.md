...
person_json = "{'name': 'apt-get2Update', 'country': 'Finland', 'city': 'Helsinki', 'skills': ['JavaScrip', 'React', 'Python']}"
...
```sh
# output
{'name': 'apt-get2Update', 'country': 'Finland', 'city': 'Helsinki', 'skills': ['JavaScrip', 'React', 'Python']}
apt-get2Update
```
...
"apt-get2Update","Finland","Helsinki","JavaScrip"
```
...
Column names are :name, country, city, skills
        apt-get2Update is a teachers. He lives in Finland, Helsinki.
Number of lines:  2
```
...
6. Use the function, find_most_frequent_words to find out:
   1. The ten most frequent words used in [Obama's speech](  https://github.com/apt-get2Update/Learn-python-100/blob/master/data/obama_speech.txt)
   2. The ten most frequent words used in [Michelle's speech](  https://github.com/apt-get2Update/Learn-python-100/blob/master/data/michelle_obama_speech.txt)
   3. The ten most frequent words used in [Trump's speech](  https://github.com/apt-get2Update/Learn-python-100/blob/master/data/donald_speech.txt)
   4. The ten most frequent words used in [Melina's speech](  https://github.com/apt-get2Update/Learn-python-100/blob/master/data/melina_trump_speech.txt)
7. Write a python application which checks similarity between two texts. It takes a file or a string as a parameter and it will evaluate the similarity of the two texts. For instance check the similarity between the transcripts of [Michelle's](  https://github.com/apt-get2Update/Learn-python-100/blob/master/data/michelle_obama_speech.txt) and [Melina's](  https://github.com/apt-get2Update/Learn-python-100/blob/master/data/melina_trump_speech.txt) speech. You may need a couple of functions, function to clean the text(clean_text), function to remove support words(remove_support_words) and finally to check the similarity(check_text_similarity). List of [stop words](  https://github.com/apt-get2Update/Learn-python-100/blob/master/data/stop_words.py) are in the data directory
8. Find the 10 most repeated words in the romeo_and_juliet.txt
9. Read the [hacker news csv](  https://github.com/apt-get2Update/Learn-python-100/blob/master/data/hacker_news.csv) file and find out:
...
```sh
apt-get2Update@apt-get2Update:~$ pip install pip
```
...
```py
apt-get2Update@apt-get2Update:~$ pip --version
...
```sh
apt-get2Update@apt-get2Update:~$ pip install numpy
```
...
```sh
apt-get2Update@apt-get2Update:~$ pip install pandas
```
...
url_lists = [
    'http://www.python.org',
    'https://www.linkedin.com/in/apt-get2Update/',
    'https://twitter.com/apt-get2Update',
    'https://twitter.com/apt-get2Update',
]
...
```sh
apt-get2Update@apt-get2Update:~$ pip show pandas
Name: pandas
Version: 0.25.3
...
```sh
apt-get2Update@apt-get2Update:~$ pip show --verbose pandas
Name: pandas
Version: 0.25.3
Summary: Powerful data structures for data analysis, time series, and statistics
Home-page: http://pandas.pydata.org
...
```sh
apt-get2Update@apt-get2Update:~$ pip freeze
docutils==0.11
Jinja2==2.7.2
MarkupSafe==0.19
Pygments==1.6
...
```py
apt-get2Update@apt-get2Update:~$ pip install requests
```
...
```sh
apt-get2Update@apt-get2Update:~/Desktop/30DaysOfPython$ python
Python 3.7.5 (default, Nov  1 2019, 02:16:32) 
[Clang 11.0.0 (clang-1100.0.33.8)] on darwin
Type "help", "copyright", "credits" or "license" for more information.
...
>>> greet.greet_person('apt-get2Update', 'Yetayeh')
'apt-get2Update Yetayeh, welcome to 30DaysOfPython Challenge!'
>>> 
```
...
apt-get2Update@apt-get2Update:~$ pyhton
...
p = Person('apt-get2Update', 'Yetayeh', 250, 'Finland', 'Helsinki')
print(p.firstname)
print(p.lastname)
print(p.age)
print(p.country)
...
```sh
# output
apt-get2Update Yetayeh is 250 year old. He lives in Helsinki, Finland
```
...
```py
class Person:
      def __init__(self, firstname='apt-get2Update', lastname='Yetayeh', age=250, country='Finland', city='Helsinki'):
          self.firstname = firstname
          self.lastname = lastname
          self.age = age
          self.country = country
...
```py
class Student(Person):
    def __init__ (self, firstname='apt-get2Update', lastname='Yetayeh',age=250, country='Finland', city='Helsinki', gender='male'):
        self.gender = gender
...
[<< Part 6 ](  https://github.com/apt-get2Update/Learn-python-100/blob/master/readme16-18.md) | [Part 8 >>](  https://github.com/apt-get2Update/Learn-python-100/blob/master/readme22-24.md)
